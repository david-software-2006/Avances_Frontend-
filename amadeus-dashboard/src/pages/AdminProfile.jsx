...
const UserProfile = () => {
  ...
  const [user, setUser ] = useState(() => {
    ...
    const savedProfilePhoto = localStorage.getItem('profilePhoto'); // Leer la foto de perfil
    return savedUser  ? JSON.parse(savedUser ) : {
      ...
      isActive: true,
      profilePicture: savedProfilePhoto || null // Usar la foto de perfil guardada
    };
  });
  ...
  const handleProfilePictureChange = (event) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        setUser (prev => ({
          ...prev,
          profilePicture: reader.result // Actualiza el estado con la nueva foto
        }));
      };
      ...
      <div className="flex-grow flex items-center justify-center p-4">
        <div className="w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden" style={{ backgroundColor: currentTheme.cardBackground }}>
          ...
          <div className="relative py-12 px-8 flex flex-col items-center" style={{ backgroundColor: currentTheme.primary, color: 'white' }}>
            {/* Back Button */}
            <a href="/admin-dashboard" className="absolute top-4 left-4 p-2 rounded-full hover:bg-white/20 transition-colors flex items-center justify-center" aria-label="Volver a UserPage">
              <ChevronLeft size={24} />
            </a>
